...
@Component({
  selector: 'app-journal-salaires',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, FormsModule],
  templateUrl: './journal-salaires.html',
  styleUrl: './journal-salaires.css'
})
export class JournalSalaires implements OnInit {
  ...
  readonly selectedThirdParty = signal<string>('');
  ...
  readonly payrollMetrics = computed(() => {
    const metrics = this.comptabiliteService.metrics();
    return metrics.byJournal.payroll;
  });

  readonly uniqueThirdParties = computed(() => {
    const entries = this.entries();
    const parties = new Map<string, { id: string; name: string }>();

    entries.forEach(entry => {
      if (entry.thirdParty && entry.thirdPartyName) {
        parties.set(entry.thirdParty, {
          id: entry.thirdParty,
    ...
    return Array.from(parties.values()).sort((a, b) => a.name.localeCompare(b.name));
  });

  readonly filteredEntries = computed(() => {
    let entries = this.entries().filter(entry => entry.journalType === JournalType.PAYROLL);
    ...
    if (this.selectedThirdParty()) {
      entries = entries.filter(entry => entry.thirdParty === this.selectedThirdParty());
    }
  ...
  async loadEntries() {
    const entries = await this.comptabiliteService.getJournalEntriesByType(JournalType.PAYROLL);
  ...
  getActiveFiltersCount(): number {
    ...
    if (this.selectedThirdParty()) count++;
  ...
  onCreateEntry() {
    console.log('Créer nouvelle écriture de salaire');
  }

  onSyncPayroll() {
    console.log('Synchroniser les paies');
  }
  ...
  onExportJournal() {
    ...
    console.log('Exporter journal des salaires avec options:', options);
  }
