...
@Component({
  selector: 'app-journal-ventes',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, FormsModule],
  templateUrl: './journal-ventes.html',
  styleUrl: './journal-ventes.css'
})
export class JournalVentes implements OnInit {
  ...
  readonly selectedCustomer = signal<string>('');
  ...
  readonly salesMetrics = computed(() => {
    const metrics = this.comptabiliteService.metrics();
    return metrics.byJournal.sales;
  });
  ...
  readonly uniqueCustomers = computed(() => {
    ...
    entries.forEach(entry => {
      if (entry.thirdParty && entry.thirdPartyName) {
        customers.set(entry.thirdParty, {
          id: entry.thirdParty,
  ...
  readonly filteredEntries = computed(() => {
    let entries = this.entries().filter(entry => entry.journalType === JournalType.SALES);
    ...
    if (this.selectedCustomer()) {
      entries = entries.filter(entry => entry.thirdParty === this.selectedCustomer());
    }
  ...
  async loadEntries() {
    const entries = await this.comptabiliteService.getJournalEntriesByType(JournalType.SALES);
  ...
  onCreateEntry() {
    console.log('Créer nouvelle écriture de vente');
  }

  onSyncInvoices() {
    console.log('Synchroniser les factures de vente');
  }
  ...
  onExportJournal() {
    ...
    console.log('Exporter journal des ventes avec options:', options);
  }
